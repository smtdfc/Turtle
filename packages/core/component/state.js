export class TurtleStateManager {
  constructor(initialState, onChangeCallback) {
    this.onChangeCallback = onChangeCallback;
    this.proxyState = this._createProxy(initialState, null, null);
  }
  ...
  _createProxy(target, parentKey, rootKey) {
    return new Proxy(target, {
      ...
      deleteProperty: (targetObj, key) => {
        const oldValue = targetObj[key];
        const fullKeyPath = parentKey ? `${parentKey}.${key}` : key;
        this.onChangeCallback(this.proxyState, "deleted", fullKeyPath, oldValue, undefined);
  ...
  set(path, value) {
    ...
    try {
      let setterFunction = new Function("value", `this.proxyState.${path} = value;`);
      setterFunction.call(this, value);
    } catch (err) {

      console.error(`Failed to set value at path: "${path}"`, err);
      throw new Error(`[State Manager Error] Error occurred while updating the state at path: "${path}".`);
    }
  }
  ...
  get(path) {
    ...
    try {
      let getterFunction = new Function("value", `return this.proxyState.${path}`);
      return getterFunction.call(this)
    } catch (err) {
      console.error(`Failed to set value at path: "${path}"`, err);
      throw new Error(`[State Manager Error] Error occurred while getting the state at path: "${path}".`);
    }
  }
  
  has(name){
    return Reflect.has(this.proxyState,name)
  }
  ...
  getStateNameFromPath(path) {
    const keys = path.split(".");
    let stateName = keys[0];
